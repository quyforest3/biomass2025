...
output_path = r"C:\Users\mn2n23\OneDrive - University of Southampton\Desktop\SC solutions (summer project)\biomass\newforrest\merged_gedi_sentinel1_sentinel2_dem_landcover_data.csv"

# Load the GEDI, SentiSnel-2, Sentinel-1, DEM, and land cover data
...
landcover_data = pd.read_csv(landcover_data_path)
...
print(dem_data.head())
print("Land Cover Data Sample:")
print(landcover_data.head())

# Ensure all datasets have common columns for merging, e.g., Latitude and Longitude
for dataset, name in [(gedi_data, 'GEDI'), (sentinel2_data, 'Sentinel-2'), (sentinel1_data, 'Sentinel-1'), (dem_data, 'DEM'), (landcover_data, 'Land Cover')]:
...
for dataset in [gedi_data, sentinel2_data, sentinel1_data, dem_data, landcover_data]:
...
landcover_gdf = gpd.GeoDataFrame(landcover_data, geometry=gpd.points_from_xy(landcover_data.Longitude, landcover_data.Latitude))
...
# Set the active geometry to use for merging with land cover data
final_merged_with_dem = final_merged_with_dem.set_geometry('geometry_gedi')

# Merge land cover data using nearest neighbor
merged_coords_landcover = np.array(list(zip(final_merged_with_dem.geometry.x, final_merged_with_dem.geometry.y)))
landcover_coords = np.array(list(zip(landcover_gdf.geometry.x, landcover_gdf.geometry.y)))

tree_landcover = cKDTree(landcover_coords)
distances_landcover, indices_landcover = tree_landcover.query(merged_coords_landcover, k=1)

nearest_landcover = landcover_gdf.iloc[indices_landcover].reset_index(drop=True)
final_merged_with_landcover = final_merged_with_dem.reset_index(drop=True).join(nearest_landcover, lsuffix='_merged', rsuffix='_landcover')

# Debug: Check if final data with land cover is merged correctly
print("Final Merged Data with Land Cover Sample:")
print(final_merged_with_landcover.head())
print("Number of samples in final merged data with land cover:", final_merged_with_landcover.shape[0])

# Check if merged data is empty
if final_merged_with_landcover.empty:
    raise ValueError("The final merged dataset with land cover is empty. Please check the merging criteria and data integrity.")

# Save the final merged data to a CSV file
final_merged_with_landcover.to_csv(output_path, index=False)
