...
# Read the first image to determine the reference shape
...
for sen1_file in sen1_files:
    try:
        resampled_data = resample_image(sen1_file, ref_shape, ref_transform, ref_crs)
        vv_bands.append(resampled_data[0])
        vh_bands.append(resampled_data[1])
    except ValueError as e:
        print(f"Skipping file {sen1_file}: {e}")

# Stack arrays along the third axis (time) and calculate the median
vv_stack = np.dstack(vv_bands)
vh_stack = np.dstack(vh_bands)
