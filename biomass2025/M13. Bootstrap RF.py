...
for i in range(n_bootstraps):
    ...
    # Initialize the model (you can choose any model, here using RandomForestRegressor)
...
print("Bootstrapped Model Performance:")
...
ax1.set_ylabel('RMSE')
ax1.set_title('Bootstrapped Random Forest Model Performance')
ax1.set_xticks(x)
...
plt.scatter(y_test, final_predictions, alpha=0.5, label='Bootstrapped RF Predictions')
plt.plot([min(y_test), max(y_test)], [min(y_test), max(y_test)], 'r--', label='Ideal Prediction Line')
plt.xlabel('True Values')
plt.ylabel('Predicted Values')
plt.title('True vs Predicted Values (Bootstrapped RF Model)')
plt.legend()
plt.grid(True)
plt.show()
...
plt.ylabel('RMSE')
plt.title('RMSE Across Bootstrap Samples (Random Forest)')
plt.grid(True)
plt.show()
...
def analyze_trends(y_true, y_pred):
    ...
    plt.title('Residuals vs Predicted Values (Bootstrapped RF)')
    plt.grid(True)
    plt.show()
